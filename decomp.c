...
/*   if (argc == 1);
                          ...
                          strncmp(argv[1],"?",1)    == 0))
     {
      fprintf(stderr,"Voyager Image Decompression Program.  Command line format:\n\n");
        ...
        if (shortint > 0 && shortint < 80)
          {
           ...
           fprintf(stderr,"This is not a VAX variable length file.");
          }
        else fprintf(stderr,"This is a VAX variable length file.");
  ...
  switch (host)
    {
     ...
     case 2: /*******************************************************/
       ...
       /* fprintf(stderr,"length=%04x,result=%d\n",length,result);          */
       nlen =   read(infile,ibuf,length+(length%2));
       return (length);
       break;
