...
#ifndef AUDIT_FILE
#define AUDIT_FILE "/usr/u1/mrvideo/lvr/lvr_gplot.dat"	/* for gplot recording audit */
#endif
...
int start_lvr (int framecopies, int startframe);
...
start_lvr (int framecopies, int startframe)
{
	char buf [100];
	...
	opt[(int)user].set = 1;
	strcpy(opt[(int)user].val.str, "mrvideo");
 	

	opt[(int)device].set=1; 
	strcpy(opt[(int)device].val.str,"Some Device?");

	opt[(int)in_name].set=1; 
	strcpy(opt[(int)in_name].val.str,"Some in_name?");

	opt[(int)title_string].set=1; 
	strcpy(opt[(int)title_string].val.str,"This is title_string?");
...
/* ***  */
   ...
   static struct code_struct_2 completes[] =
       { { SEARCH,1,"SEARCH"},
	 {	REPEAT,3,"REPEAT"},  /* ...or 2 sometimes */
...
{
	char play=PLAY, splay=SLOW_PLAY, fplay=FAST_PLAY, still=STILL, stop=STOP
		, menu=MENU, contin=CONTINUE,
		rsteps=REV_STEP_STILL,fsteps=FWD_STEP_STILL,
		fstep=FWD_STEP, repeat=REPEAT,

		clear=CLEAR, eject=EJECT, frameMode=FRAME_MODE,

		addrInq=ADDR_INQ, curBlank=CURRENT_BLANK_INQ,

		search=SEARCH, rec_stand=REC_STANDBY, rec=REC,edgeR=EDGE_RIGHT,
		edgeL=EDGE_LEFT, blankS=BLANK_AREA_SEARCH, readD=READ_DATA,
		setD=SET_DATA, writeD=WRITE_DATA, enter=ENTER, noOp=NO_OP;
	int i = atoi (argv [1]);
	char  com [299];
	start_lvr (1, 1);
	
	sprintf(com,"%c%d%c",SEARCH, i, ENTER);
	if(output_cmds(com,strlen(com))==COMPLETION)
		printf("search of %d OK\n", i);
	else
		printf("search of %d not OK\n", i);
}
